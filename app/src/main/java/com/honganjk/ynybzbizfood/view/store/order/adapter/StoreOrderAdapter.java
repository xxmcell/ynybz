...
public class StoreOrderAdapter extends CommonAdapter<StoreOrderData2.ObjsBean> {
    ...
    public void convert(final ViewHolder holder, final StoreOrderData2.ObjsBean data) {
        ...
        setRecyclerView(lists, data, Data, holder);
...
//        "全部", "0待付款", "1待发货", "2待收货", "3待评价"
    ...
    private void setRecyclerView(List<List<StoreOrderData2.ObjsBean.DetailsBean>> lists, StoreOrderData2.ObjsBean data, StoreOrderData2.ObjsBean.DetailsBean.ListBean data1, ViewHolder holder) {
        RecyclerView recyclerView = holder.getView(mrecyclerView);
    ...
    private class myAdapter extends RecyclerView.Adapter {
        List<List<StoreOrderData2.ObjsBean.DetailsBean>> lists;

        public myAdapter(StoreOrderFragment mContext, List<List<StoreOrderData2.ObjsBean.DetailsBean>> lists,
                         StoreOrderData2.ObjsBean data, StoreOrderData2.ObjsBean.DetailsBean.ListBean data1) {
            this.lists=lists;
        }
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (viewType == 0) {
                View titalView= LayoutInflater.from(MyApplication.getContext()).inflate(R.layout.store_item_orders,null,false);
                return new titleViewHolder(titalView);
            }
            if(viewType==1){
                View normalView=LayoutInflater.from(MyApplication.getContext()).inflate(R.layout.store_item_order_items,null,false);
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
                if(position!=0){
                    lists.get(0).get(0).getFeature();
                    int itemViewType=getItemViewType(position);
                    if(itemViewType==0){
                        gettitleItem((titleViewHolder) holder,lists.get(0).get(0));
                    }else if(itemViewType==1){
                        getNormalItem((normalViewHolder) holder, lists.get(0).get(position-1));
                    }
                }
        }

        private void getNormalItem(normalViewHolder holder,StoreOrderData2.ObjsBean.DetailsBean detailsBean) {
            for (int i = 0; i < detailsBean.getList().size(); i++) {

            }
         //   holder.theType.setText();
        }

        private void gettitleItem(titleViewHolder holder,StoreOrderData2.ObjsBean.DetailsBean detailsBean) {
            titleViewHolder titleType=holder;
            titleType.title.setText(detailsBean.getFeature());
        }

        @Override
        public int getItemCount() {
            return lists.get(0).size()+1;
        }
        ...
        private class titleViewHolder extends RecyclerView.ViewHolder {
            ...
            public titleViewHolder(View titleView) {
                super(titleView);
                title = titleView.findViewById(R.id.name);
            }
        }

        private class normalViewHolder extends RecyclerView.ViewHolder {

            private final TextView theType;

            public normalViewHolder(View normalView) {
                super(normalView);
                theType = normalView.findViewById(R.id.type);
            }
        }
    }
}
