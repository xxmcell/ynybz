 ...
 * //设置标题
 * dialogDateTime.setTitleMain("派工选择");
 * <p>
 * //设置最小时间
 * Calendar minTime = Calendar.getInstance();
 * dialogDateTime.setMinCalendar(minTime);
 * <p>
 * //设置最大时间
 * Calendar maxTime = Calendar.getInstance();
 * maxTime.add(Calendar.MONTH, 1);
 * dialogDateTime.setMaxCalendar(maxTime);
 * <p>
 * //显示
 * dialogDateTime.show();
 * //回调
 ...
 *
 * @Override public void onClick(DialogDateTime dialog, int number, Calendar content) {
 * time.setText(content.get(Calendar.YEAR) + "-" + (content.get(Calendar.MONTH) + 1)
 * + "-" + content.get(Calendar.DAY_OF_MONTH)
 * + " " + content.get(Calendar.HOUR_OF_DAY)
 * + ":" + content.get(Calendar.MINUTE)
 * );
 * }
 * });
 * <p>
 * 设置初始化时间
 * dialogDateTime.setInitTime(minTime);
 */
...
public class DialogDateTime extends DialogFragment {
    ...
    public interface OnClickCallback {
        void onClick(DialogDateTime dialog, int number, Calendar content);
    }
...
//        getWindow().setAttributes(p);
    ...
    public void quedingOnClick(View view) {
        if (onClickCallback != null) {
            onClickCallback.onClick(DialogDateTime.this, 2,
                    getTime());
        }
        dismiss();
    }
