...
//            "V", "W", "X", "Y", "Z"};
    ...
    private void init() {
        ...
        textColor = getResources().getColor(R.color.main_color);
        // 设置粗体
    ...
    public void addAllChar(List<CityData> c) {
        letter.clear();
        for (CityData cc : c) {
            if (!letter.contains(cc.getInitial())) {
                letter.add(cc.getInitial());
            }
        }
        if (!letter.contains("#"))
            letter.add("#");
    ...
    private void setPaintTextSize(int height) {
        ...
        if (letterHeight >= 20) {
            paint.setTextSize(DimensUtils.sp2px(getContext(), 12));
            paintSelect.setTextSize(DimensUtils.sp2px(getContext(), 12 + 5));
        } else {
            paintSelect.setTextSize(DimensUtils.sp2px(getContext(),
                    letterHeight / 2 + 2 + 5));
            paint.setTextSize(DimensUtils.sp2px(getContext(),
                    letterHeight / 2 + 2));
        }
